...
def ensure_state():
    ss = st.session_state
    ss.setdefault("session_name", f"New Chat {datetime.now().strftime('%H-%M')}")
...
def list_sessions():
    try:
        ...
        files.sort(key=lambda f: os.path.getctime(os.path.join(HISTORY_DIR, f)), reverse=True)
...
def sanitize_name(s: str) -> str:
    s2 = "".join(c for c in s if c.isalnum() or c in (" ", "_", "-")).strip()[:MAX_NAME]
...
def stream_reply(messages):
    try:
        for chunk in ollama.chat(model=MODEL, messages=messages[-MAX_KEEP:], stream=True):
...
def on_send():
    ...
    final = (f"You are an assistant that answers based on the provided context. "
...
def on_new_chat():
    st.session_state.session_name = f"New Chat {datetime.now().strftime('%H-%M')}"
        ...
        with row:
            ...
            with c2:
                if st.button("⋮", key=f"opt_{name}"):
                    ...
                    st.session_state.menu_open = {k: False for k in st.session_state.menu_open}
...
if st.session_state.file:
    with st.container():
        ...
        with colA:
            if getattr(st.session_state.file, "type", "").startswith("image/"):
                st.image(st.session_state.file, caption="Image attached")
            elif getattr(st.session_state.file, "type", "") == "application/pdf":
                st.info(f"📄 PDF attached: `{st.session_state.file.name}`")
        with colB:
            st.button("Clear", use_container_width=True, on_click=on_clear_file)
